...
namespace CatFeederWatchdog
{
    public class DisplayService
    {
        ...
        private const int lineHeight = (einkHeight - padding * 2) / 5;
        ...
        public async Task DisplayFeedInfoAsync(DateTime lastFeed, DateTime nextFeed, bool isAlert, string message)
        {
            ...
            RectangleF msgLine = new RectangleF(padding, padding + 3 * lineHeight, einkWidth - 75, lineHeight);
        ...
        static IEnumerable<byte> ZipBytes(IEnumerable<byte> bytes)
        {
            ...
            for (int i = 0; i < cnt; i += 8)
            {
                ...
                enumerator.MoveNext();
                var byte4 = enumerator.Current == 0 ? 0 : 1 << padding;
                enumerator.MoveNext();
